...
import { buildGoogleAuthUrl, handleApiError, requestRegisterOtp, verifyRegisterOtp } from "@/lib/api";
...
export default function RegisterPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await verifyRegisterOtp({ name, email, password, code });
      ...
      <div className="space-y-8 rounded-3xl border border-black/10 bg-white p-8 shadow-[0_14px_60px_rgba(12,22,44,0.08)]">
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <Button type="submit" variant="primary" full disabled={status.loading}>
            {step === "request" ? "Send OTP" : "Verify & create account"}
          </Button>
          {step === "verify" && (
            <Button
              type="button"
