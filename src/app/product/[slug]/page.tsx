  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const reviewData = await fetchReviews(data.id);
        setReviews(reviewData);
        if (token) {
          const wishlist = await fetchWishlist();
          setWishlisted(wishlist.some((item: { productId: string }) => item.productId === data.id));
        }
      } catch (err) {
    ...
    return (
      <div className="mx-auto max-w-3xl px-6 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-gray-600">
          {error || "We couldn’t fetch this product. Try again or browse the collection."}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      ...
      <div className="mt-14">
        ...
        />
        <div className="mt-6">
          <ProductGrid products={related} loading={!related.length} />
        </div>
      </div>
          ...
          <div className="rounded-3xl border border-black/10 bg-white p-6 shadow-[0_14px_60px_rgba(12,22,44,0.08)]">
            {token ? (
              ...
              >
                ...
                />
                {reviewStatus.error && (
                  <p className="text-sm text-rose-500">{reviewStatus.error}</p>
                )}
